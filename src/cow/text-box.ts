...
export class TextBox {
  ...
  private _createCanvas = (): CanvasRenderingContext2D => {
    registerFont(resolve(this._fontDir + this._props.font.fileName), { family: this._props.font.family });
  ...
  public getPositionedText = (xOffset: number, yOffset: number, xCenter: boolean = true, yCenter: boolean = true, inclLeadingSpace: boolean = false): PositionedTextLine[] => {
    ...
    return this._lines.map((line, index) => {
      ...
      const hSpaceOffset: number = inclLeadingSpace && leadingSpace.length ? this._ctx.measureText(leadingSpace).width : 0;
      const xPos: number = xOffset + (xCenter ? Math.floor((this._props.width - line.width) / 2) : 0) + hSpaceOffset;
