...
import { COW_DB, COW_TYPES, PHONE_TYPES, PROD } from "../constants";
...
export abstract class Cow {
  ...
  public speak(text: string, wrap: boolean = true): boolean {
    ...
    if (this._textBox.isTextFitting()) {
      console.log("I will MOO!", { text: oneLiner, lines: this._textBox.getLines().length, boxSize: this._textBox.getTextSize() });
      return true;
    }
    console.error("Holy cow! That won't fit into my speech bubble...", { text: oneLiner, lines: this._textBox.getLines().length, boxSize: this._textBox.getTextSize() });
    return false;
  }
...
export class Os40Cow extends Cow {
  ...
  public generate = async (): Promise<string> => {
    ...
    // Save as bitmap file
    const writeProcess = async () => new Promise<string>((resolve, reject) => {
      image.toBuffer((error: Error | null, buffer: Buffer) => {
        if (error) {
          ...
          const dir: string = PROD ? "./" : "./.ftp/";
          const bmp: string = this._makeFileName("bmp")
          writeFile(dir + bmp, buffer, () => resolve(bmp))
        }
      });
    });
